...
Before evaluating cards, determine what archetype the deck is pursuing. Check `run-state.md` (if it exists) for a prior assessment. If not, assess now by examining the player's deck composition (visible from relics, powers, and cards seen in combat).
...
- If the best card scores positively: `./sts2 choose_card <reward_index> <card_index>`
- If all cards score negatively (wrong archetype, deck too big, mediocre cards): `./sts2 skip_card <reward_index>`
